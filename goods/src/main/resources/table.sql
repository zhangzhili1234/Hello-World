--mysql建表模板
--先drop 后create
--主键使用id，字段名中"_"两端不能使用单字符
--时间对象使用int(10),时间戳/1000
--在建表时指定索引，外键必须加上索引，非空的字段必须加上非空属性
--指定表引擎 innodb
drop table if exists mall_url;
create table if not exists mall_url
(
    id          	bigint(20) primary key comment '主键',
    url             varchar(100) not null comment 'url',
    is_access       varchar(1) not null comment '是否可用 0：可用，1：不可用',
    is_login        varchar(1) not null comment '是否需要登陆 0：需登陆，1：无需登录',
